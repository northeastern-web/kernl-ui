<div class="k_grid k_bg-light">
  <section class="section">
    <header class="k_section__header">Base Grid</header>
    <div class="g-row">
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --5@md">
        <div class="col-content">5</div>
      </div>
      <div class="g-col --12@xs --7@md">
        <div class="col-content">7</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --6@md">
        <div class="col-content">6</div>
      </div>
      <div class="g-col --12@xs --6@md">
        <div class="col-content">6</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --7@md">
        <div class="col-content">7</div>
      </div>
      <div class="g-col --12@xs --5@md">
        <div class="col-content">5</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --8@md">
        <div class="col-content">8</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --9@md">
        <div class="col-content">9</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --10@md">
        <div class="col-content">10</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --11@md">
        <div class="col-content">11</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs">
        <div class="col-content">12</div>
      </div>
    </div>
  </section>

  <hr>

  <section class="section">
    <header class="k_section__header">Grid, No Gutters</header>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --5@md">
        <div class="col-content">5</div>
      </div>
      <div class="g-col --12@xs --7@md">
        <div class="col-content">7</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --6@md">
        <div class="col-content">6</div>
      </div>
      <div class="g-col --12@xs --6@md">
        <div class="col-content">6</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --7@md">
        <div class="col-content">7</div>
      </div>
      <div class="g-col --12@xs --5@md">
        <div class="col-content">5</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --8@md">
        <div class="col-content">8</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --9@md">
        <div class="col-content">9</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --10@md">
        <div class="col-content">10</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs --11@md">
        <div class="col-content">11</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
    </div>
    <div class="g-row g--nogutters">
      <div class="g-col --12@xs">
        <div class="col-content">12</div>
      </div>
    </div>
  </section>

  <hr>

  <section class="section">
    <header class="k_section__header">Grid, Banners, Nesting, Fancy Options and Tweaks</header>
    <h3 class="non-col-element text-style-block">Section</h3>
    <div class="g-row">
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
      <div class="g-col --12@xs --1@md">
        <div class="col-content">1</div>
      </div>
    </div>
  </section>
  <section class="section --banner">
    <h3 class="non-col-element text-style-block">Section <small>(with --banner option)</small></h3>
    <div class="g-row">
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
    </div>
  </section>
  <section class="section">
    <h3 class="non-col-element text-style-block">Section</h3>
    <div class="g-row">
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content">3</div>
      </div>
    </div>
  </section>
  <section class="section --banner">
    <h3 class="non-col-element text-style-block">Section <small>(with --banner option)</small></h3>
    <div class="g-row">
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4</div>
      </div>
    </div>
  </section>
  <section class="section">
    <h3 class="non-col-element text-style-block">Section</h3>
    <div class="g-row">
      <div class="g-col --12@xs --5@md">
        <div class="col-content">5</div>
      </div>
      <div class="g-col --12@xs --7@md">
        <div class="col-content">7</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --6@md">
        <div class="col-content">6</div>
      </div>
      <div class="g-col --12@xs --6@md">
        <div class="col-content">6</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --7@md">
        <div class="col-content">7</div>
      </div>
      <div class="g-col --12@xs --5@md">
        <div class="col-content">5</div>
      </div>
    </div>
    <div class="g-row +middle">
      <div class="g-col --12@xs --8@md">
        <section class="section --banner">
          <h3 class="non-col-element text-style-block">Nested Section <small>(with --banner option)</small></h3>
          <div class="g-row">
            <div class="g-col --12@xs --4@md">
              <div class="col-content">4</div>
            </div>
            <div class="g-col --12@xs --4@md">
              <div class="col-content">4<br>----<br>-------</div>
            </div>
            <div class="g-col --12@xs --4@md">
              <div class="col-content">4</div>
            </div>
          </div>
        </section>
      </div>
      <div class="g-col --12@xs --4@md">
        <div class="col-content">4 (+middle on row)</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --9@md">        
        <div class="col-content" style="padding: 0;">
          <section class="section">
            <h3 class="non-col-element text-style-block">Nested Section</h3>
            <div class="g-row">
              <div class="g-col --12@xs --3@md">
                <div class="col-content">3</div>
              </div>
              <div class="g-col --12@xs --3@md">
                <div class="col-content">3<br>----<br>-------<br>----<br>-------</div>
              </div>
              <div class="g-col --12@xs --3@md">
                <div class="col-content">3<br>----<br>-------</div>
              </div>
              <div class="g-col --12@xs --3@md">
                <div class="col-content">3</div>
              </div>
            </div>
          </section>
        </div>
      </div>
      <div class="g-col --12@xs --3@md">
        <div class="col-content +equal">3 (.equal-height)</div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs --10@md">
        <div class="col-content">10</div>
      </div>
      <div class="g-col --12@xs --2@md">
        <div class="col-content">2</div>
      </div>
    </div>
  </section>
  <section class="section --banner">
    <h3 class="non-col-element text-style-block m--b-1@xs">Section <small>(with --banner option)</small></h3>
    <div class="section --banner">
      <h3 class="non-col-element text-style-block">Nested Section <small>(with --banner option)</small></h3>
      <div class="g-row">
        <div class="g-col --12@xs --11@md">
          <div class="col-content">11</div>
        </div>
        <div class="g-col --12@xs --1@md">
          <div class="col-content">1</div>
        </div>
      </div>
    </div>
    <div class="g-row">
      <div class="g-col --12@xs">
        <div class="col-content">12</div>
      </div>
    </div>
  </section>
</div>